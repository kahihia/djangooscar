...
class OrderPlacementMixin(CheckoutSessionMixin):
    ...
    #: "checkout.views.paymentDetailView" 에서 사용
    """
    ...
    def handle_order_placement(self, order_number, user, basket,
                               ...
                               **kwargs):
        """
        basket이 frozen상태일때 일어난다
        order model을 쓴다 그리고 return http response
    ...
    def place_order(self, order_number, user, basket, shipping_address,
        ...
        # billing_address가 None이 아닐때 저장한다
        billing_address = self.create_billing_address(
                billing_address, shipping_address, **kwargs)

        # order_status 저장 없으면 get_initial_order_status에 서 가져온다 (basket)에
    ...
    def save_payment_details(self, order):
        """
        모든 payment-related detail을 저장한다

        "save_payment_event(order)-
        "save_payment_source(order) -

        billing address, payment source 등등
    ...
    def handle_successful_order(self, order):
        ...
        # session에 checkout_order_id에 저장한다
        self.request.session['checkout_order_id'] = order.id

        # redirect한다 get_success_url로

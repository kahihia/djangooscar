...
_CategoryForm = movenodeform_factory(
    Category,
    fields=['name', 'description', 'image', 'cn_name','cn_description','cn_image'])

class CategoryForm(_CategoryForm):


    def save(self, commit=True):
        cate_form = super(CategoryForm,self).save(commit=False)
        if cate_form.cn_name:
            cate_form.is_chinese = True
        else:
            cate_form.is_chines=False
...
class ProductForm(forms.ModelForm):
    FIELD_FACTORIES = {
        ...
        'description_img': _attr_image_field,

    }

    def save(self, commit=True):
        super_form = super(ProductForm, self).save(commit=False)
        if super_form.cn_title:
            super_form.is_chinese = True
        else:
            super_form.is_chinese = False
    ...
    class Meta:
        model = Product
        fields = [
            'title', 'upc', 'description', 'description_img', 'is_discountable', 'structure', 'cn_title',
            'cn_description']

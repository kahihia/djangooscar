...
class BasketRemoveItemView(View):
    def post(self, request, *args, **kwargs):
        checked_item = request.POST.get('checked')
        checked_item = checked_item.split(',')

        basket_model = request.basket

        _basket_line_model = get_model('basket', 'Line')

        for i in checked_item:
            _basket_line_model.objects.filter(basket_id=basket_model.id, product_id=i).delete()
